...
/**
 * Serialize Message
 * @param {WAConnection} lz 
 * @param {Object} m 
 * @param {Boolean} hasParent 
 */
exports.smsg = (lz, m, hasParent) => {
  if (!m) return m
  ...
  if (m.key) {
    ...
    m.sender = m.fromMe ? lz.user.jid : m.participant ? m.participant : m.key.participant ? m.key.participant : m.chat
  }
  if (m.message) {
    m.mtype = Object.keys(m.message)[0]
    m.msg = m.message[m.mtype]
    if (m.mtype === 'ephemeralMessage') {
      exports.smsg(lz, m.msg)
      m.mtype = m.msg.mtype
      m.msg = m.msg.msg
    }
    ...
    if (m.quoted) {
      ...
      m.quoted.fromMe = m.quoted.sender === (lz.user && lz.user.jid)
      m.quoted.text = m.quoted.text || m.quoted.caption || ''
      m.quoted.mentionedJid = m.quoted.contextInfo ? m.quoted.contextInfo.mentionedJid : []
      m.getQuotedObj = m.getQuotedMessage = async () => {
        if (!m.quoted.id) return false
        let q = await lz.loadMessage(m.chat, m.quoted.id)
        return exports.smsg(lz, q)
      }
      ...
      if (m.quoted.url) m.quoted.download = (type = 'buffer') => lz.downloadM(vM, type)
      /**
       ...
       * @param {Object} options 
       */
      m.quoted.reply = (text, chatId, options) => lz.reply(chatId ? chatId : m.chat, text, vM, options)
      /**
       * Copy quoted message
       */
      m.quoted.copy = () => exports.smsg(lz, M.fromObject(M.toObject(vM)))
      /**
       * Forward quoted message
       * @param {String} jid 
       * @param {Boolean} forceForward 
       */
      m.quoted.forward = (jid, forceForward = false) => lz.forwardMessage(jid, vM, forceForward)
      /**
       ...
       * @param {Object} options 
       */
      m.quoted.copyNForward = (jid, forceForward = false, options = {}) => lz.copyNForward(jid, vM, forceForward, options)
      /**
       ...
       * @param {Object} options 
       */
      m.quoted.cMod = (jid, text = '', sender = m.quoted.sender, options = {}) => lz.cMod(jid, vM, text, sender, options)
      /**
       * Delete quoted message
       */
      m.quoted.delete = () => lz.deleteMessage(m.quoted.chat, vM.key)
    }
    if (m.msg.url) m.download = (type = 'buffer') => lz.downloadM(m, type)
     ...
     * @param {Object} options 
     */
    m.reply = (text, chatId, options) => lz.reply(chatId ? chatId : m.chat, text, m, options)
    /**
     * Copy this message
     */
    m.copy = () => exports.smsg(lz, M.fromObject(M.toObject(m)))
    /**
     * Forward this message
     * @param {String} jid 
     * @param {Boolean} forceForward 
     */
    m.forward = (jid = m.chat, forceForward = false) => lz.forwardMessage(jid, m, forceForward)
    /**
     ...
     * @param {Object} options 
     */
    m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => lz.copyNForward(jid, m, forceForward, options)
    /**
     * Modify this Message
     * @param {String} jid 
     * @param {String} text 
     ...
     * @param {Object} options 
     */
    m.cMod = (jid, text = '', sender = m.sender, options = {}) => lz.cMod(jid, m, text, sender, options)
  }
  return m
}
